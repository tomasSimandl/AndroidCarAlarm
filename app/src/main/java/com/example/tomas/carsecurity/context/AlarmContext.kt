...
class AlarmContext(private val sharedPreferences: SharedPreferences, private val context: Context) {
    ...
    /** Contains default interval for which is next detection ignored after detection. Value is taken from resources. */
    private val defIgnoreAlarmInterval :Long = context.resources.getInteger(R.integer.default_alarm_ignore_interval).toLong()
    ...
    /**
     * Returns default interval for which is next detection ignored after detection.
     * Value is taken from shared preferences or it is used default value.
     */
    val ignoreAlarmInterval
        get() = sharedPreferences.getLong(context.getString(R.string.key_alarm_ignore_interval), defIgnoreAlarmInterval)
