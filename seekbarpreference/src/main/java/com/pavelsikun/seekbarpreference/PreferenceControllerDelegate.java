    ...
    void loadValuesFromXml(AttributeSet attrs) {
        if(attrs == null) {
            ...
            try {
                ...
                int resourceKey = attrs.getAttributeResourceValue("http://schemas.android.com/apk/res/android", "defaultValue", -1);
                if(resourceKey == -1){
                    currentValue = DEFAULT_CURRENT_VALUE;
                } else {
                    try {
                        currentValue = context.getResources().getInteger(resourceKey);
...
//                dialogStyle = a.getInt(R.styleable.SeekBarPreference_msbp_interval, DEFAULT_DIALOG_STYLE);
    ...
    public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
        int newValue = minValue + progress;
